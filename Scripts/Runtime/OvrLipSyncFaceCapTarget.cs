...
public class OvrLipSyncFaceCapTarget : MonoBehaviour
{
    // PUBLIC
    ...
    public FaceCapVisemes faceController = null;
    ...
    void Start ()
    {
        ...
        if(!faceController)
        {
            Debug.LogError("LipSyncContextMorphTarget.Start Error: " +
                "Please set the target controller to be controlled!");
            return;
        }
    ...
    void Update ()
    {
        if(lipsyncContext && faceController)
        {
    ...
    void SetVisemeToMorphTarget(OVRLipSync.Frame frame)
    {
        for (int i = 0; i < visemeToBlendTargets.Length; i++)
        {
            if (visemeToBlendTargets[i] != -1)
            {
                ...
                faceController.SetBlendShapeWeight(
